...
import { isNetworkOfflineError } from '@/lib/offline-api';
...
/**
 * Offline-aware session bootstrap:
 * - If no token → not authenticated
 * - If offline AND token exists → use cached session (allow offline usage)
 * - If online → verify token with server (as usual)
 * - If online but network error → fall back to cached session
 */
export const bootstrapSession = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    ...
    // Offline mode: trust the stored session
    if (!navigator.onLine) {
    ...
    try {
      ...
      // Network unreachable (not a 401): fall back to cached session
      if (isNetworkOfflineError(error)) {
        return getStoredSessionUser();
      }
      // 401 or other server error: clear session
...
export const signIn = createAsyncThunk(
  'auth/signIn',
  async (
    payload: { username: string; password: string; category: string },
    { rejectWithValue },
  ) => {
    try {
